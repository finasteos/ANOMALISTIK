...
function shannonEntropy(counts, n) {
  if (n <= 0) return 0;
  let h = 0;
...
function indexOfCoincidence(counts, n) {
  if (n <= 1) return 0;
  let num = 0;
...
// src/server/rng.ts
var RNG_TAU = 0.2;
var RNG_HIST_BINS = 40;
function directionalBF01(observedD, stdPermD, tau = RNG_TAU) {
...
function adjudicateVerdict(args) {
  if (!args.layer1Passed) return "INSTRUMENT_SYSTEMATICS";
  if (args.pValue < 1e-3 && args.observedD > 0) return "STRUCTURE_SIGNAL";
  if (args.pValue < 0.05 && args.observedD > 0) return "UNDERDETERMINED";
  return "CLAIM_FAILS_NULL";
}
function buildHistogram(values, observed, nBins = RNG_HIST_BINS) {
...
// src/server/routes/rng.ts
function createRngRouter(opts) {
  const router = express.Router();
  const DATA_RNG_DIR2 = opts.dataDir;
  const COMMITMENTS_PATH = path.join(DATA_RNG_DIR2, ".commitments.json");
  const activeSessionCommitments = /* @__PURE__ */ new Map();
  function loadCommitments() {
    try {
      if (!fs.existsSync(COMMITMENTS_PATH)) return;
      const raw = JSON.parse(fs.readFileSync(COMMITMENTS_PATH, "utf-8"));
  ...
  function persistCommitment(sessionId, entry) {
    try {
      let raw = {};
      ...
      console.warn("[rng] commitment journal write failed:", e?.message);
    }
  }
  function getCommitment(sessionId) {
  ...
  loadCommitments();
  function countBufferOnes(buffer) {
    return countOnesBytes(buffer);
  }
  function generateDeterministicBuffer(seed, blockIndex, length) {
    ...
    while (offset < length) {
      ...
      counterBuf.writeUInt32BE(blockIndex * 1e5 + counter, 0);
    ...
    try {
      ...
      return res.json({
        localNode: {
          ...
          isAppleSilicon,
          role: isAppleSilicon ? "Silicon Processing Unit (M-Series)" : "Control Station / Analysis Host (iMac Pro Intel)",
  ...
  router.post("/session/start", (req, res) => {
    try {
      ...
      persistCommitment(sessionId, entry);
  ...
  router.post("/session/analyze", (req, res) => {
    try {
      ...
      const intentionScores = blocks.filter((b) => b.condition === "INTENTION").map((b) => Number(b.targetScoreZ) || 0);
      const controlScores = blocks.filter((b) => b.condition === "CONTROL").map((b) => Number(b.targetScoreZ) || 0);
      const meanIntentionZ = intentionScores.length > 0 ? intentionScores.reduce((a, b) => a + b, 0) / intentionScores.length : 0;
      const meanControlZ = controlScores.length > 0 ? controlScores.reduce((a, b) => a + b, 0) / controlScores.length : 0;
      const observedD = meanIntentionZ - meanControlZ;
      const totalBits = blocks.reduce((acc, b) => acc + (b.nBits || 0), 0);
      const totalOnes = blocks.reduce((acc, b) => acc + (b.ones || 0), 0);
      const overallRawZ = totalBits > 0 ? (2 * totalOnes - totalBits) / Math.sqrt(totalBits) : 0;
      const nPermutations = 2e4;
      ...
      const sessionFile = path.join(DATA_RNG_DIR2, `${sessionId}.json`);
  ...
  const CHAIN_PATH = path.join(DATA_RNG_DIR2, "sessions.jsonl");
  function appendChainEntry(sessionId, sessionConfig, analysis) {
    try {
      let prev = "GENESIS";
      if (fs.existsSync(CHAIN_PATH)) {
        ...
        if (lines.length > 0) {
          try {
            prev = JSON.parse(lines[lines.length - 1]).hash || "GENESIS";
          } catch {
          }
        }
      }
      ...
      console.warn("[rng] chain append failed:", e?.message);
    }
  }
  router.get("/sessions", (req, res) => {
    try {
      if (!fs.existsSync(DATA_RNG_DIR2)) {
        return res.json([]);
      }
      const limit = Math.min(Math.max(parseInt(String(req.query.limit ?? "50"), 10) || 50, 1), 200);
      const offset = Math.max(parseInt(String(req.query.offset ?? "0"), 10) || 0, 0);
      const files = fs.readdirSync(DATA_RNG_DIR2).filter((f) => f.endsWith(".json")).map((f) => {
        const filePath = path.join(DATA_RNG_DIR2, f);
        try {
...
// src/server/routes/geospace.ts
import express2 from "express";
import path2 from "path";
import fs2 from "fs";
function createGeospaceRouter(opts) {
  const router = express2.Router();
  const DATA_SYNCED_DIR2 = opts.syncDir;
  const DATA_DOWNLOADS_DIR2 = opts.downloadsDir;
  function latestFile(dir, pattern) {
    try {
      const files = fs2.readdirSync(dir).filter((f) => pattern.test(f)).map((f) => ({ name: f, mtime: fs2.statSync(path2.join(dir, f)).mtimeMs })).sort((a, b) => b.mtime - a.mtime);
      return files[0] ? path2.join(dir, files[0].name) : null;
    } catch {
      return null;
    }
  }
  router.get("/", (_req, res) => {
    try {
      const csvPath = latestFile(DATA_SYNCED_DIR2, /geospace.*\.csv$/);
      if (!csvPath) {
      ...
      return res.json({
        source_file: path2.basename(csvPath),
        rows_total: rows.length,
        rows_returned: tail.length,
        last_updated: fs2.statSync(csvPath).mtime.toISOString(),
        snapshot: {
  ...
  router.get("/status", (_req, res) => {
    try {
      const csvPath = latestFile(DATA_SYNCED_DIR2, /geospace.*\.csv$/);
      const manifest = latestFile(DATA_SYNCED_DIR2, /manifest.*\.json$/);
      const dscovr = latestFile(path2.join(DATA_DOWNLOADS_DIR2, "dscovr"), /\.parquet$/);
      const eida = latestFile(path2.join(DATA_DOWNLOADS_DIR2, "eida"), /\.parquet$/);
      const imag = latestFile(path2.join(DATA_DOWNLOADS_DIR2, "intermagnet"), /\.parquet$/);
      const age = (p) => {
        if (!p) return null;
        try {
          const ms = Date.now() - fs2.statSync(p).mtimeMs;
...
function createAiRouter() {
  ...
  async function callOpenRouter(messages, options = {}) {
    ...
    const timeout = setTimeout(() => ctrl.abort(), 25e3);
    let res;
    try {
  ...
  router.post("/search-grounded", async (req, res) => {
    try {
      ...
      const safeQuery = String(query).slice(0, 4e3);
  ...
  router.post("/high-thinking", async (req, res) => {
    try {
      ...
      const safePrompt = String(prompt).slice(0, 8e3);
...
// src/server/routes/adjudicate.ts
import express4 from "express";
import crypto2 from "crypto";
function createAdjudicateRouter() {
  ...
  router.post("/", (req, res) => {
    ...
    const cleanSeq = sequence.trim().slice(0, 5e4);
...
// src/server/routes/declassified.ts
import express5 from "express";
import fs3 from "fs";
function createDeclassifiedRouter(opts) {
  const router = express5.Router();
  const DECLAS_SAMPLES_DIR = opts.samplesDir;
  const DECLAS_CATALOG_PATH = opts.catalogPath;
  if (fs3.existsSync(DECLAS_SAMPLES_DIR)) {
    router.use("/images", express5.static(DECLAS_SAMPLES_DIR));
  }
  router.get("/catalog", (_req, res) => {
    try {
      if (fs3.existsSync(DECLAS_CATALOG_PATH)) {
        const data = JSON.parse(fs3.readFileSync(DECLAS_CATALOG_PATH, "utf-8"));
...
var DATA_SYNCED_DIR = path3.join(process.cwd(), "data", "synced");
var DATA_DOWNLOADS_DIR = path3.join(process.cwd(), "data", "downloads");
var DATA_RNG_DIR = path3.join(process.cwd(), "data", "rng_sessions");
if (!fs4.existsSync(DATA_RNG_DIR)) {
  fs4.mkdirSync(DATA_RNG_DIR, { recursive: true });
}
app.use(express6.json({ limit: "10mb" }));
app.disable("x-powered-by");
app.use(securityHeaders);
app.use("/api/ai/", rateLimit(30, 6e4));
app.use("/api/", rateLimit(300, 6e4));
app.use((req, _res, next) => {
  if (process.env.VERCEL === "1") {
    const url = req.url.split("?")[0];
    const isAsset = /\.[a-zA-Z0-9]+$/.test(url);
    if (!req.url.startsWith("/api") && !req.url.startsWith("/@") && !req.url.startsWith("/src") && url !== "/health" && !isAsset) {
...
app.use(
  ...
  createDeclassifiedRouter({
    samplesDir: path3.join(process.cwd(), "data", "declassified_sample", "images"),
    catalogPath: path3.join(process.cwd(), "data", "declassified_archive_index.json")
  })
);
